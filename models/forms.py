...
class CommentForm(Form):
    ...
    name = StringField('name', [validators.data_required(), validators.length(max=20)])
    email = StringField('email', [validators.optional(), validators.length(min=5, max=50)])
    content = TextAreaField('content', [validators.data_required(), validators.length(min=5, max=1000)])
